...
#                                                                           #
#    Copyright 2020 Inorganic Materials Chemistry                           #
...
void StructureRenderer::draw(const Structure *structure, bool periodicity_xy, bool periodicity_z) {
    ...
    if(structure->get_nr_atoms() < 1000) {
...
void StructureRenderer::draw_movement_plane(const Structure* structure) {
    ...
    QVector3D v1 = this->scene->rotation_matrix.inverted() * QVector3D(1.0f, 0.0f, 0.0f);
    QVector3D v2 = this->scene->rotation_matrix.inverted() * QVector3D(0.0f, 0.0f, 1.0f);
